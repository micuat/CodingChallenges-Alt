...
function Lane() {
  if (arguments.length == 3) {
    ...
    this.type = t;
    let offset = sketch.random(0, 255);
    let obsc;
    if (this.type == CAR) {
      this.col = sketch.color(255);
      obsc = sketch.color(255, 0, 0);
    }
    else if (this.type == LOG) {
      this.col = sketch.color(0, 0, 255);
      obsc = sketch.color(255, 255, 0);
    }
    for (let i = 0; i < n; i++) {
      this.obstacles[i] = new Obstacle(sketch, offset + spacing * i, index * grid, grid * len, grid, speed, obsc);
    }
  }
}
