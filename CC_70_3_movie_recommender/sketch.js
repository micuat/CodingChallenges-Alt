...
var s = function (sketch) {
  ...
  sketch.preload = function () {
    data = sketch.loadJSON('movies.json');
  }
  ...
  sketch.setup = function () {
    ...
    for (var i = 0; i < titles.length; i++) {
      var div = sketch.createDiv(titles[i]);
      var dropdown = sketch.createSelect('');
    ...
    var button = sketch.createButton('submit');
    ...
    function findNearestNeighbors(user) {
      ...
      for (var i = 0; i < data.titles.length; i++) {
        ...
        if (user[title] == null) {
          ...
          var stars = sketch.nf(weightedSum / similaritySum, 1, 2);
          var div = sketch.createDiv(title + ': ' + stars);
          resultDivs.push(div);
          div.parent(resultP);
